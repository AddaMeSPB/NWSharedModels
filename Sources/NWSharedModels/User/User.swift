...
public final class UserModel: Model {
    ...
    public init(
		id: ObjectId? = nil,
		fullName: String,
		language: UserLanguage, role: UserRole = .basic,
...
extension UserModel {
	public func mapGet() -> UserGetObject {
		return .init(id: id, fullName: fullName, email: email, role: role, language: language)
	}
...
extension UserGetObject: Content {
    public init(from user: UserModel) {
        self.init(id: user.id, fullName: user.fullName, email: user.email, role: user.role, language: user.language)
    }
}
#endif
